...
[事実] 現在のrepositoryには、Go API、Browser UI、Rust/GCP legacy、iOS実装途中コード、mobile関連docsが混在している。
...
既存のRust/GCP legacy、iOS途中実装、Android設計メモや途中成果物は、active source treeには残さず、archive branchに退避する。
...
- Rust/GCP legacy API active CI/CD
...
- Rust/GCP legacy codeとmobile途中成果物はarchive branchで参照可能にする。
...
1. Rust/GCP/iOS/Android legacyを含むarchive branchを作成または更新する。
2. 本repositoryからRust/GCP legacy deploy証跡とmobile active source treeを削除するcleanup branchを作る。
