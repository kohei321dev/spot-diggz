...
4. 初期画面ではYouTube iframe、外部サムネイル、player APIをloadしない。利用者が「YouTubeで動画を見る」を明示操作した後にだけ、1件のiframeを遅延作成する。`autoplay=0` とし、利用者の明示操作なしに再生しない。embedを表示する領域はYouTubeの最低表示サイズを満たす。
5. iframeを作成する前に、YouTubeへの通信と再生時の追加通信が起こり得ることをUIで明示する。`Referrer-Policy` は埋込playerがclient identityを受け取れる `strict-origin-when-cross-origin` とし、`noreferrer` を使わない。privacy-enhanced endpointでも、第三者送信がないとは表現しない。
6. iframeが利用できない、埋込が禁止されている、動画が削除・非公開になった、または利用者が埋込を選ばない場合、元のYouTube watch URLを新しいtabで開く外部リンクを表示する。外部リンクは `noopener` を付け、動画の閲覧は推薦・詳細表示を失敗させない。
...
### 初期表示でYouTubeサムネイルとiframeをloadする

動画の存在を強く訴求できるが、結果表示時点でYouTubeへの通信が発生し、初期表示性能とprivacyが悪化する。明示操作後の遅延loadを採用する。
...
- 明示操作後はYouTubeへ第三者通信が発生し、provider側の表示・広告・利用規約に依存する
...
- 初期結果表示時にYouTube iframe、YouTube thumbnail request、player API requestがないこと
- 動画操作後だけiframeを1件作成し、`autoplay=0`、16:9表示、YouTube attributionと標準player controlを維持すること
