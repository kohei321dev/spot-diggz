...
| R-014 | 施設ごとに手動キュレーション済みYouTube動画を最大1件、任意の補助情報として表示できる | 滑走環境を想像し、候補を比較しやすくするため | catalogで検証済みのYouTube動画だけを施設ごとに0または1件表示する。利用者が動画表示を明示操作するまでiframe・外部サムネイルを読まず、自動再生しない。埋込不可・削除・通信失敗時は推薦表示を継続し、通常のYouTube外部リンクへ縮退する |
...
- 手動キュレーション済みYouTube動画の明示操作後表示と、公式確認済みInstagram・Xプロフィールへの外部リンク
...
- YouTube iframeは利用者の明示操作後に第三者通信を発生させる。providerの規約、telemetry、保持期間はowner確認が必要である
