...
1. **技術スキル向上**: バックエンド（Go）、フロントエンド（TypeScript/React）の習得
...
- **言語**: Go
- **アプローチ**: Skate Spot Metadata APIとして責務を絞る
- **legacy**: 既存Rust実装は移行完了まで `web/api/` に保持する

**選定理由**:

- **単一バイナリ**: CI/CD、artifact管理、container buildと接続しやすい
- **標準toolchain**: `go fmt` / `go vet` / `go test` / `go build` / `govulncheck` を使いやすい
- **保守性**: Rustスクラッチ実装よりAPI本体の実装量と運用負荷を抑えやすい
