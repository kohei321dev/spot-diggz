...
## 現行プロダクト方針（2026-07-12）

このリポジトリは、既存の地図・スポット検索機能をそのまま拡張するのではなく、
「今日どこで、どのように滑るか」の意思決定を支援する多言語Webサービスとして再検証する。
現行方針の基準は [docs/product_baseline.md](docs/product_baseline.md) とする。

- 主対象は、都市部の初心者・復帰者。副対象は来訪者・訪日スケーター。
- 初期提供範囲は、開発者が情報を検証できる1地域と20〜30施設に限定する。
- Webを先行し、ネイティブiOS/Androidアプリは初期検証の対象外とする。
- 施設情報を根拠にした決定的な絞り込み・スコアリングを先に実装し、AIは自然言語入力、理由説明、比較、翻訳を補助する。
- AIは営業時間、料金、利用可否、ルール、位置情報を推測・創作してはならない。回答には検証済みデータと出典を利用する。
- Instagramの自動収集、未検証のストリートスポット投稿、全国規模の一括登録、独自地図・ナビゲーションは、利用実態を確認するまで実装しない。
- 正確な現在地は原則として保存しない。必要な場合も、利用目的・保存期間・削除方法を明示する。
- 既存のiOS/Android/Rust/GCP実装は、移行方針が決まるまで削除しない。

開発の進め方は [docs/development_workflow.md](docs/development_workflow.md)、設計判断は
[docs/adr/](docs/adr/) を参照する。下記に残る旧来の学習目標・大規模GCP構成は過去の検討内容であり、
現行MVPの必須要件ではない。新しい実装や設計を追加する際は、プロダクト要求、Issue、またはAccepted ADRとの対応を明示する。
...
- コミット前に、Rustユニットテスト/TypeScript(UI)ユニットテスト/`.github/workflows/ci.yml` 記載のテストがローカルで全てPASSしていることを確認すること。
...
graph TD
    ...
    I[ログ集約プロジェクト<br/>sdz-logs] --> J[BigQuery Dataset<br/>audit_logs]
    A -.->|組織レベルログシンク| I
```
...
| **ログ集約**       | `sdz-logs`                           | 組織全体のログ・監査データ集約                        | セキュリティ監視の実装       |
...
gcloud logging sinks create org-audit-sink \
  bigquery.googleapis.com/projects/sdz-logs/datasets/audit_logs \
  --organization=$ORG_ID \
  --include-children \
...
gcloud budgets create \
  ...
  --threshold-rule=percent=50 \
  --threshold-rule=percent=90 \
  --notification-rule=pubsub-topic=projects/sdz-logs/topics/budget_alerts
```

**命名規約**: CLAUDE.md「命名規約（4層ティア）」を参照。
...
graph TD
    A[組織レベルログ] --> B[ログシンク]
    B --> C[BigQuery Dataset<br/>sdz-logs.audit_logs]
...
gcloud logging sinks create security-alert-sink \
  pubsub.googleapis.com/projects/sdz-logs/topics/security_alerts \
  --organization=$ORG_ID \
  --include-children \
...
FROM `sdz-logs.audit_logs.cloudaudit_googleapis_com_activity_*`
