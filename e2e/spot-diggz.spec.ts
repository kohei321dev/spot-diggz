...
test("キュレーション済み動画を初期表示して開閉でき、集計eventに対象情報を送らない", async ({
  ...
  await primaryCard.getByRole("button", { name: "動画を表示", exact: true }).click();
  await expect(primaryCard.getByRole("button", { name: "動画を閉じる", exact: true })).toHaveCount(1);
  await expect(iframe).toBeVisible();

  await expect.poll(() => productEventPayloads.map((payload) => payload.event)).toEqual(
    expect.arrayContaining(["video_embed_displayed", "video_embed_loaded"]),
  );
